...
a1 <- ggplot(data, aes(x = `ES_xt value`, y = Outcome)) +
  geom_pointrangeh(
    aes(
      xmin = LCI,
      xmax = UCI,
...
### 绘图：Model dodge + mycolor
a2 <- ggplot(data, aes(x = `ES_xt value`, y = Outcome)) +
  geom_pointrangeh(
    aes(
      xmin = LCI,
      xmax = UCI,
...
### 绘图：Model dodge + mycolor
a3 <- ggplot(data, aes(x = `ES_xt value`, y = Outcome)) +
  geom_pointrangeh(
    aes(
      xmin = LCI,
      xmax = UCI,
  ...
  geom_vline(xintercept = 0, linetype = "dashed", linewidth = 1.3, color = "#8e8c8c") +
  scale_x_continuous(limits = c(-0.035, 0.08)) +
  scale_color_identity() +  # 使用数据框中的颜色
  theme_minimal() +
  labs(x = "β value (95% CI)", y = "") +
  theme(
...
top_part <- ggarrange(
  ...
  align = "h",          # 水平对齐坐标轴
  widths = c(1,1,1.2)     # 强制每个 panel 宽度相等
)
top_part
# 保存PDF
ggsave(
  filename = paste0(path_data,"lmm_LC3_Cog_fo.pdf"),
  plot = top_part,
